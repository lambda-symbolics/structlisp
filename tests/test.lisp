...
(define-test test-deque-move-if
  (let ((source (make-deque :initial-capacity 4
                            :initial-contents '(1 2 3 4 5)
                            :weight-function #'identity))
        (target (make-deque :initial-contents '(0)
                            :maximum-count 3)))
    (deque-pop-front source)
    (deque-push-back source 6)
    (multiple-value-bind (result evicted moved)
        (deque-move-if #'evenp source target)
      (assert (eq target result))
      (test-equal #(0) evicted)
      (test-equal #(2 4 6) moved))
    (test-equal '(3 5) (deque->list source))
    (test-equal 8 (deque-total-weight source))
    (test-equal '(2 4 6) (deque->list target)))
  (let ((source (make-deque :initial-contents '((1 odd) (2 even) (3 odd))))
        (target (make-deque)))
    (deque-move-if #'evenp source target :key #'first)
    (test-equal '((1 odd) (3 odd)) (deque->list source))
    (test-equal '((2 even)) (deque->list target)))
  (let ((source (make-deque :initial-contents '("aa" "bbb" "c")
                            :weight-function #'length))
        (target (make-deque :initial-contents '("old")
                            :maximum-weight 5
                            :weight-function #'length
                            :eviction-end :back)))
    (multiple-value-bind (ignored evicted moved)
        (deque-move-if (lambda (value) (<= (length value) 2)) source target)
      (declare (ignore ignored))
      (test-equal #("c") evicted)
      (test-equal #("aa" "c") moved))
    (test-equal '("bbb") (deque->list source))
    (test-equal 3 (deque-total-weight source))
    (test-equal '("old" "aa") (deque->list target))
    (test-equal 5 (deque-total-weight target)))
  (let ((calls 0)
        (deque (make-deque :initial-contents '(1 2))))
    (multiple-value-bind (result evicted moved)
        (deque-move-if (lambda (element)
      ...
      (test-equal #() moved))
    (test-equal 0 calls)
    (test-equal '(1 2) (deque->list deque))))

(define-test test-deque-move-if-validates-before-mutation
  (let ((source (make-deque :initial-contents '(good bad untouched)))
        (target (make-deque
    ...
    (handler-case
        (progn
          (deque-move-if (lambda (element)
                           (member element '(good bad)))
                         source target)
          (error "Expected target weight validation to fail."))
      (deque-weight-error ()))
    (test-equal '(good bad untouched) (deque->list source))
    (assert (deque-empty-p target)))
  (let* ((calls 0)
         (source (make-deque :initial-contents '(a b)))
         (target (make-deque
                  :weight-function (lambda (element)
                                     (declare (ignore element))
                                     (when (= (incf calls) 2)
    ...
    (handler-case
        (progn
          (deque-move-if (constantly t) source target)
          (error "Expected late target callback failure."))
      (simple-error (condition)
        (setf failure
              (search "late target failure" (princ-to-string condition)))))
    (assert failure)
    (test-equal '(a b) (deque->list source))
    (assert (deque-empty-p target)))
  (let* ((calls 0)
         (armed-p nil)
         (source (make-deque
    ...
    (handler-case
        (progn
          (deque-move-if (lambda (element) (= element 1)) source target)
          (error "Expected late source callback failure."))
      (simple-error (condition)
        (setf failure
              (search "late source failure" (princ-to-string condition)))))
    (assert failure)
    (test-equal '(1 2 3) (deque->list source))
    (assert (deque-empty-p target)))
  (let ((source (make-deque :initial-contents '(1 2 3)))
        (target (make-deque :initial-contents '(old)))
        (failure nil))
    (handler-case
        (progn
          (deque-move-if (lambda (element)
                           (if (= element 2)
                               ...
                               (oddp element)))
                         source target)
          (error "Expected predicate failure."))
      (simple-error (condition)
        (setf failure
              (search "predicate failure" (princ-to-string condition)))))
    (assert failure)
    (test-equal '(1 2 3) (deque->list source))
    (test-equal '(old) (deque->list target))))
